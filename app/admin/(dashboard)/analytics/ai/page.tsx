...
export default function AiAnalyticsPage() {
  ...
  return (
    <div className="space-y-10">
      ...
      {overview?.anomalies &&
        (overview.anomalies.cost_spike || overview.anomalies.failure_spike) && (
          <div className="p-4 rounded-2xl bg-destructive/10 border border-destructive/20 flex items-center gap-3 text-destructive">
            ...
            <div className="text-sm font-medium">
              {overview.anomalies.cost_spike &&
                "Anomalous cost spike detected today. "}
              {overview.anomalies.failure_spike &&
                "Higher than normal AI failure rate detected."}
            </div>
          </div>
        )}
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="border-border shadow-none rounded-lg">
          ...
          <CardContent>
            <div className="text-3xl font-heading font-bold">
              ${overview?.ai_stats.total_cost.toFixed(2)}
            </div>
        ...
        <Card className="border-border shadow-none rounded-lg">
          ...
          <CardContent>
            <div className="text-3xl font-heading font-bold">
              ${overview?.ai_stats.avg_cost_per_article.toFixed(4)}
            </div>
            <p className="text-[10px] text-muted-foreground mt-1 uppercase font-bold tracking-tighter">
              Efficiency metric
            </p>
        ...
        <Card className="border-border shadow-none rounded-lg">
          ...
          <CardContent>
            <div className="text-3xl font-heading font-bold">
              {overview?.ai_stats.success_rate}%
            </div>
            <p className="text-[10px] text-muted-foreground mt-1 uppercase font-bold tracking-tighter">
              Reliability
            </p>
        ...
        <Card className="border-border shadow-none rounded-lg">
          ...
          <CardContent>
            <div className="text-3xl font-heading font-bold">
              {aiUsage?.total_requests.toLocaleString()}
            </div>
            <p className="text-[10px] text-muted-foreground mt-1 uppercase font-bold tracking-tighter">
              Request volume
            </p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <div className="h-4 w-full bg-secondary/50 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${((aiUsage?.total_tokens.input || 0) / ((aiUsage?.total_tokens.input || 0) + (aiUsage?.total_tokens.output || 1))) * 100}%`,
                  }}
                />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="h-4 w-full bg-secondary/50 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${((aiUsage?.total_tokens.output || 0) / ((aiUsage?.total_tokens.input || 0) + (aiUsage?.total_tokens.output || 1))) * 100}%`,
                  }}
                />
              </div>
            </div>
        ...
        <Card className="border-border shadow-none rounded-lg">
          <CardHeader>
            ...
            <CardDescription>
              Distribution of spend across AI capabilities.
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-4 space-y-6">
            {costBreakdown?.cost_by_feature &&
              Object.entries(costBreakdown.cost_by_feature).map(
                ([feature, cost]) => (
                  <div
                    key={feature}
        ...
        <CardContent className="h-64 flex items-end gap-2 pt-6">
          {costBreakdown?.cost_by_day.map((day, i) => {
            const maxCost = Math.max(
              ...costBreakdown.cost_by_day.map((d) => d.cost),
              1,
            );
          ...
          {(!costBreakdown?.cost_by_day ||
            costBreakdown.cost_by_day.length === 0) && (
